...
class TokenizerManager:
    ...
    async def _handle_single_request(
        ...
        if not is_cache_for_prefill:  # The normal case with a single prompt
            ...
            #     logger.debug(f"RID: {rid}, Text: {decoded_text}, Input IDs: {input_ids}")
            ...
            # if logger.isEnabledFor(logging.DEBUG):
            #     decoded_text = self.tokenizer.decode(input_ids)
            #     logger.debug(f"Prefill RID: {rid}, Text: {decoded_text}, Input IDs: {input_ids}")
    ...
    async def _wait_for_response(
        ...
        while True:
            ...
            if self.server_args.log_requests and state.finished:
                ...
                # logger.debug(f"in={obj}, Decoded Text: {decoded_text}, out={out}")
